...
namespace Sres.Net.EEIP
{
	public class EEIPClient
	{
		...
		{
			var eipClient = new EEIPClient();
			List<Encapsulation.CIPIdentityItem> cipIdentityItem = eipClient.ListIdentity();
			var lines = new List<string>();

			foreach (var device in cipIdentityItem)
			{
				lines.Add("Ethernet/IP Device Found:");
				lines.Add(device.ProductName1);
				lines.Add("IP-Address: " + Encapsulation.CIPIdentityItem.getIPAddress(device.SocketAddress.SIN_Address));
				lines.Add("Port: " + device.SocketAddress.SIN_port);
				lines.Add("Vendor ID: " + device.VendorID1);
				lines.Add("Product-code: " + device.ProductCode1);
				lines.Add("Type-Code: " + device.ItemTypeCode);
				lines.Add($"Status: {device.Status1}");
				lines.Add($"State: {device.State1}");
				lines.Add($"Item Length: {device.ItemLength}");
				lines.Add("\n");
			}
		...
		public bool IsTcpClientConnected()
		{
			try
			{
				...
				else if (!client.Client.Connected) return false;
				else
					// Detect if client disconnected
					return (client.Client.Available > 0 || !client.Client.Poll(1, SelectMode.SelectRead));
				//else if (socket.IsBound) return !_terminate;
			}
			catch (ObjectDisposedException ode)
			{
				SocketDisposed?.Invoke(ode);
			}
			catch (Exception ex)
			{
				SocketException?.Invoke(ex);
			}
			return false;
		}

		public event Action<ObjectDisposedException> SocketDisposed;
		public event Action<Exception> SocketException;
		...
		public uint ForwardOpen()
		{
			return this.ForwardOpen(false);
		}

		public uint ForwardOpen(bool largeForwardOpen)
		{
